...
> **【2026-07-24 新增·知识消费侧（用户提议 + AI 归类至本阶段）】** 周总结 / 活研究报告 / 文档浏览器 三者构成「用学到的东西」界面层，落点统一在**巨作总指挥部**（`pages/` 新增页面，直接读熔知数据），不塞进凝华。边界（AI 设计决策，非用户铁律；方向已于 2026-07-24 经用户确认）：活研究报告 = 用户主动设主题的活文档（随知识库更新完善 + 列缺失项）；阶段 4「知识关系网」= 系统自动发现知识点关联——二者不重叠、不增殖。
...
| 活研究报告 + 文档浏览器 | 用户设定研究主题 → 生成「活文档」，随熔知知识库更新自我完善并列出缺失项；文档浏览器可浏览熔知向量库内文件并做笔记 | 巨作 `pages/` | ⚪ 待命 |
| 能力保留 | 凝华现有能力（酷家乐 SOP、时间审计、小红书代发）保持独立可卖 | 凝华 | ✅ 现状 |

> **【2026-07-25 页面整合·已落地】** 原 3 个子页面（总仪表盘 / 开发进度 / 项目连接器）的去重内容已合并为单一「🎛️ 总指挥部」(`pages/1_🎛️_总指挥部.py`)，旧 3 页已删除。健康检查仅留一份、GitHub API 速查仅一份、任务看板用开发进度超集（含筛选+统计）；侧边栏外链改读 `settings` 修正炼真端口写死 8503 的 bug。周看板后续作为巨作**首页** `app.py`（见上方周总结行）。
