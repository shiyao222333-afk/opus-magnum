...
- 拉起炼真 / 熔知时，带环境变量 **`ALBEDO_KEEP_INPUT=1`** 和 **`KB_KEEP_INBOX=1`**
  → 炼真不把中转①移走、熔知不删中转②，防止误删。
- 平时不带这两个变量（用 `start_all.bat` 等）→ 原行为恢复。**开关式，不是运行时热切。**
...
5. 任选 1 份炼真文件提交熔知 **3 次**，每次记录 **53 字段结果**后删测试数据 → **3 份字段文件**（文件5/6/7）。
6. 共 **7 份文件**。
...
1. **熔知内容去重**：若熔知对「完全相同的正文」做内容级去重（而非按文件名），第 2/3 次提交同一份炼真文件可能不新摄入 → 流程会在该步超时报错。届时需确认熔知去重策略，或调整本流程为「轻微改文件名内容」以触发 3 次独立摄入。
