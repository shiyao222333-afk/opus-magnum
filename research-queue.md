...
- [ ] **【和你同行的对照组：一个建筑效果图 AI SaaS 从 150 美元/月做到 8600 美元 MRR，转折点不是模型而是交付形态】** | 来源：[案例复盘](https://dy.163.com/article/L35U0UQE05566BKF.html) | 🎯 Rubedo 凝华
  白天上班晚上做产品的单人创始人，做建筑效果图 SaaS，提供节点式操作和多种生成能力，技术不弱，但两年卡在约 150 美元/月。原因不是模型差，而是**建筑师不想先把自己变成技术人员**——他们要的是一张能拿去提案的图，不是一套控制面板。他做了三件事：把节点界面改成普通对话（描述空间/风格/改动即可出图）、改成订阅制、围绕高意图搜索词获客，之后自报 MRR 达 8600 美元。这条和 SSS 的本行几乎完全同构，值得当模板拆：**先把一个复杂功能压缩成三句话——客户输入什么、收到什么结果、多久能用于下一步工作**；如果客户还需要你解释半小时，说明交付没被产品化。注意数字是 Reddit 自报未经审计，学的是那个转折逻辑，不是那个金额。

- [ ] **【小米 MiMo-V2.5 全系 MIT 开源 + API 永久降价最高 99%，另有 30 天百万亿 Token 免费计划】** | 来源：[央视报道转述](https://k.sina.com.cn/article_7879849460_1d5acf5f406801cnwc.html) | 🎯 Citrinitas 熔知
  8/2 雷军转发央视报道称其登顶全球大模型调用量第一。对一人公司真正有用的是三条硬指标：**MIT 全量开源**（权重+代码，可自由商用/微调，无需报备授权）；API 输入/输出降到每百万 Token 1 元 / 2 元，缓存命中低至 2 分钱，且官方称同任务比 Claude/Gemini 少烧 40%-60% Token；**MiMo Orbit 计划 30 天内发放总计百万亿 Token 免费额度，单人最高 16 亿 Credits**。熔知的批量摘要、打标、嵌入前处理这类高频低难度调用，本周就可以拿免费额度做一轮完整回放对比，不行再切回去——零成本试错窗口是有期限的，别拖过 30 天。

- [ ] **【模型路由代运维：给中小企业配 OpenRouter/LiteLLM，月费 $2K-$7K、毛利约 78%，"工作是配置不是编码"】** | 来源：[打法与利润拆解](https://betonai.net/multi-model-ai-routing-side-hustle-2026-2k-7k-month-openrouter-litellm-smbs-margin-math) | 🎯 Rubedo 凝华
  逻辑很直白：**同一个任务在不同模型间的价差是当前 AI 经济里最大的持续套利空间，而小企业自己没能力吃到**。交付物是一套路由配置 + 每月 2-4 小时的调优（拉上月用量报告 → 找出哪些任务可以换更便宜的模型或哪里质量掉了 → 改路由规则 → 发一页纸报告）。两种收费形态：客户自持 API Key 你只做"路由+月度调优"，或你全托管客户只看一个账单。熟练者 5-6 个客户约 12-15 小时/月的存量工作量。SSS 手上已有 7/03 起累积的整条路由降本知识（提供商评估、缓存路由、Luna 档批量任务），**这条是把已有知识直接换钱的最短路径，且不需要写代码**。

### 🟡 关注

- [ ] **【AI Agent 服务的真实报价表：月费 $1200-$15000 分四档，按结果计价 $45-180/会议，一次性交付是陷阱】** | 来源：[五种模式的 MRR 数据](https://betonai.net?p=1740/) | 🎯 Rubedo 凝华
  给接单定价用的锚：①**留存制**（最赚）Starter $1200-2500（1 个 Agent+周会）/ Growth $2500-5000 / Operator $5000-8000（含 SLA）/ Fractional Head of AI $8000-15000；②**按结果计价**（增长最快，因为客户觉得像投广告不像请顾问）每场约到的会 $45-180、每条合格线索 $8-35、每张处理的发票 $0.25-1.10；③产品化 Agent $39-199/月走长尾；④白标给代理机构，$1500-4000 建设费 + $400-1500/月授权；⑤**一次性定制交付 $2000-12000 是明确的陷阱**——月流水最高的那批操盘手都说除非第二个月能转成留存制，否则不接。到 $10K MRR 的现实路径是**3-5 个客户复用同一个 Agent 模板**，不是接一个大单。

- [ ] **【视频号 8 月创作者分层 2.0：从看粉丝量改看综合评分，亿元级流量补贴砸向中腰部】** | 来源：[闭门会信号整理](http://m.woshizmt.cn/m/view.php?aid=3257) | 🎯 Rubedo 凝华
  7 月底微信创作者闭门会流出的说法：8 月起分层从粉丝量改为"内容质量分+互动健康分+商业履约分+用户粘性分"复合评分，划 S/A/B/C/D 五级，**B 级（60-75 分）占创作者总量约 42%**，升级后自动获得"中腰部"标识，拿流量加权、商单优先推荐、直播流量券。提到的门槛是完播率 >45%、互动率 >5%、评论回复率 >60%。**这是二手转述、未见官方公告，先按"待验证信号"处理**；但方向和 7/31 收的"四平台集体转向中长干货视频"一致——如果属实，对粉丝量不大但内容扎实的账号是明确利好，SSS 做效果图/家装干货正好卡在这个位置。本周先做的是那个几乎零成本的动作：**把评论回复率拉起来**。

- [ ] **【抖音 7.8 新规落地：推流窗口从 24 小时拉到 7 天，收藏和二刷取代点赞成核心指标；带货服务费按 A-D 分级 6%-40%】** | 来源：[新规解析](https://m.sohu.com/a/1047600387_122510839) + [费率改革背景](https://www.woshipm.com/share/6411698.html) | 🎯 Rubedo 凝华
  两条独立但同向的变化。**算法侧**：取消 24 小时短期数据判定，改七天长效推送；把收藏量、二次浏览量作为核心推流依据，点赞权重大幅降低——直接后果是**"有用到想收藏"的垂类干货吃长期流量，博眼球的同质化内容流量衰减**，同时平台降低头部倾斜，给中小垂类留了口子。**带货侧**：统一 10% 费率终结，按合规/内容/履约/结算综合表现月度评级，A 级低至 6%（顶级可全额返还）、B 级 10%、C 级 13%-20%、D 级最高 40%；橱窗门槛提到 500 粉。对 SSS 的意义不在带货，而在**"收藏率"是效果图内容天然的强项**——把笔记做成"可存下来对照施工的清单/参数图"，比做成好看的展示图更吃现在的算法。

- [ ] **【腾讯元宝 Agent 免费出 PPT/Word/Excel/PDF/HTML 且不限次数，对标的是豆包最高档 5088 元/年】** | 来源：[实测对比](https://new.qq.com/rain/a/20260801A038UK00?refer=cp_1009) | 🎯 Rubedo 凝华
  元宝接入混元 Hy3 后上线 Agent 能力，文件生成全免费无次数限制；同类办公任务模式在豆包专业版最高档是一年 5088 元。实测提到 1 分钟出 PPT、微信内转发即处理、四引擎可切。对一人公司就是一句话：**方案汇报、报价单、客户交付文档这类"格式重于智力"的活儿，没必要再付订阅费**。本周花 20 分钟拿一份真实的项目汇报试一次，能顶用就把这块支出砍掉；顶不用也只损失 20 分钟。

- [ ] **【省额度的实操：让贵模型当工头、便宜模型当工人，Codex 里配子代理把产出翻倍】** | 来源：[aihot](https://x.com/AYi_AInotes/status/2083867265179537565) | 🎯 Rubedo 凝华
  具体做法是在 `~/.codex/agents/` 下建一个 `luna-worker.toml` 子代理，模型设成便宜档、推理强度拉满，**旗舰模型只负责拆任务和审代码，具体实现全部委托下去**。这是"模型路由"思路从 API 层下沉到日常 vibe coding 的版本，Claude Code 里可以用同样思路手工模拟（主会话做规划和 review，实现交给便宜模型的子会话）。和 7/30 收的"堆叠会话拆大重构"是一对：**一个管范围不失控，一个管额度不烧穿**。

### ⚪ 了解

- [ ] **【RoomGPT 开源版（MIT，10.7k star）：ControlNet 先抓房间骨架再换皮，自部署单张推理约 0.005-0.02 美元】** | 来源：[工具评测](https://chdh.me/tools/ai-tools/image/roomgpt) | 🎯 Rubedo 凝华
  了解备查。技术路线值得记：**先用 ControlNet 提取原图房间结构线稿，再让扩散模型在保留几何骨架的前提下换软装/灯光/材质**，所以不会出现墙窗错位——这是它和直接 text-to-image 工具的关键差别。SaaS 版售价 0.145-0.30 美元/张，开源版自接 Replicate 只付推理费约 0.005-0.02 美元/张。这条补上了 8/2 那条"海外出图报价跌到 0.40 美元/张"缺的成本底线数据：**别人的售价底线约 0.15 美元，自建成本约 0.01 美元**。它做不了精确尺寸（那是酷家乐/Coohom 的活），定位是"一张照片十秒出方案图"。

- [ ] **【价格战的反向信号：DeepSeek/OpenAI 在降，Kimi K3 和 Seedance 2.5 API 反而在涨】** | 来源：[7 月模型市场综述](https://www.163.com/dy/article/L3BPRD8405198CJN.html) + [AI 智能体日报](https://new.qq.com/rain/a/20260801A038UK00?refer=cp_1009) | 🎯 Citrinitas 熔知
  同一周内：OpenAI 把 Luna 砍 80%、Terra 砍 20%、旗舰 Sol 一分不降还加了双倍价的 Fast 模式；DeepSeek V4-Flash 正式版输入 0.14 / 输出 0.28 美元每百万 Token 且 MIT 开源；**但 Kimi K3 涨价、Seedance 2.5 API 涨 50%**。结论不是"AI 越来越便宜"，而是**文本推理在坍塌、视频生成在涨价、旗舰能力坚决不降**。做成本模型时要分开算：熔知的文本管线可以按"持续变便宜"规划，任何依赖视频生成的内容管线要按"可能继续涨"留缓冲。

### 💡 探索

- [ ] **【券商开始形成共识：模型层利润归零，价值向应用层转移】** | 来源：[财联社 8/2 综述](https://www.163.com/dy/article/L3BPRD8405198CJN.html) | 🎯 OpusMagnum 巨作
  国联民生、东方证券、国金证券在 7 月底同时给出同一判断：**AI 投资战略重心正向应用方向倾斜，因为这是回答"资本开支能不能换回产业资金回流"的唯一出路**。逻辑链是：开源模型能力快速逼近闭源 → 切换成本极低（改两行配置就能换 API）→ 生态锁定失效 → 价格成为最敏感变量 → 模型层利润被持续挤压 → 竞争转移到"谁能把 AI 用好"。对巨作总指挥部的意义：**这不是利好某个项目，而是确认了整体押注方向没错**——熔知/馏析/炼真/凝华全都在应用层，且都建立在"随时可切换模型"的假设上，这个假设正在被整个行业验证为正确。反过来的警示是：应用层的护城河也不能是"我接了某个模型"，只能是数据、工作流和客户关系。
...
- [ ] **【OpenWorker：吴恩达 7/23 开源的「本地优先」桌面 Agent，MIT 许可，接 Ollama 可做到零 API 费】** | 来源：[GitHub](https://github.com/andrewyng/openworker) + [产品解析](https://www.usecarly.com/blog/openworker-andrew-ng) | 🎯 Rubedo 凝华
  吴恩达和 Rohit Prasad 联手发的桌面 Agent，跟聊天机器人的区别在于**它交付成品**——一份编辑好的文档、一条更新的日历、一条带具体数据的 Slack 消息，而不是一段回复。架构全在本机：Tauri 2 外壳 + React 前端 + Python FastAPI 本地服务（127.0.0.1:8765），自带文件系统 / Git / ripgrep 搜索 / Shell / 待办清单工具，支持 MCP，每个任务默认最多 12 轮模型-工具迭代，做任何有后果的动作前会先问你。**模型完全中立**：GPT-5.6 / Claude / Gemini / Kimi / GLM / DeepSeek 都行，走 Ollama 就是彻底零成本。目前只有 Mac 版，Windows 版官宣「即将」——SSS 是 Windows，先盯 release，Windows 一落地立刻装。赚钱价值：这是**唯一一个"跑在自己电脑上、不按月付订阅费"的成品级 Agent 底座**，凝华的 SOP 自动化可以直接架在它上面，省掉每月几十美元的托管 Agent 订阅。

- [ ] **【模型路由服务化：给中小企业部署 OpenRouter/LiteLLM 路由层，单客户月费 $400–$3,500，实测账单砍 78%】** | 来源：[Bet on AI 完整拆解](https://betonai.net/multi-model-ai-routing-side-hustle-2026-2k-7k-month-openrouter-litellm-smbs-margin-math) | 🎯 Rubedo 凝华
  队列里已经攒了十几条模型路由的**技术**条目（Weave Router / Wayfinder / Cursor Router / Prompt Caching…），这条第一次把它变成**可以卖钱的服务包**。核心卖点是一张账单对照表：5000 万 token/月的典型中小企业负载，单一高端模型 $75，按任务类型路由后 $16.40，降 78%——批量摘要走 Gemini Flash、抽取走 DeepSeek、决策留高端、敏感数据走本地模型。四种标准报价：一次性配置 $1,200–2,400；配置+月度调优 $800–1,500 起 + $400–900/月；托管路由（你持 key）$2,000–3,500 + $900–2,200/月；分时 AI 运维 $3,000–5,000 + $1,500–3,500/月。单人天花板约 6 个托管客户 / 月入 $5,200–7,400。**关键是它卖的是配置不是编码**，SSS 的 vibe coding 能力足够覆盖，且交付物就是熔知/凝华已经跑通的那套东西。附带 30 天获客计划和"绝不绑单一厂商"的定位话术。

- [ ] **【抖音「AI+真人」短剧双重补贴，窗口 7/1–9/30：成片保底最高 300 万/部，后验分成单月最高 30 万】** | 来源：[骨朵 AI 政策拆解](https://new.qq.com/rain/a/20260722A0FQC800?refer=cp_1009) + [短剧版权中心征集](https://d.youth.cn/newtech/202604/t20260421_16619509.htm) | 🎯 Rubedo 凝华
  平台一个月内第二次给「AI+真人」赛道砸钱，是**正在开着的窗口**，不是回顾性资讯。硬门槛很明确：总时长 ≥100 分钟、真人实拍占比 ≥20%、AI 生成内容（场景/特效/道具/数字角色）占比 ≥5%，题材不限。收益是两层叠加：前置保底按评级 S 级 100 万 / A+ 级 60 万 / A 级 40 万（影视 IP 改编成片可到 300 万/部）；后置按热度和有效播放时长再给 3%–20% 阶梯分成，单部单月封顶 30 万，结算周期上线后 90 天。平台的态度写得很白——**现阶段要的是"真人为主、AI 为辅"**，AI 是用来补宏大外景、奇幻特效、特殊道具这些"传统实拍成本高到拍不起"的部分。对 SSS 的实际切口不是自己下场拍剧，而是**做那 5%–20% 的 AI 场景/道具/特效外包**：这正好是效果图 + 3D 白模 + Seedance 2.5 白模参考（8/02 已收）的组合拳，制作团队缺的就是这个能力。

### 🟡 关注

- [ ] **【Hermes Agent：Nous Research 开源自进化 Agent，自己写 MEMORY.md、自己造 skill，$5 VPS 就能跑】** | 来源：[产品拆解](https://kie.ai/pt/blog/what-is-hermes-agent) + [GitHub](https://github.com/NousResearch/hermes-agent) | 🎯 Rubedo 凝华
  MIT 许可，当前 v0.18.2（7/7）。它的定位不是 IDE 里的编码副驾，而是一个**长期活着的进程**：闭环学习——从经验里自动生成 skill，用的过程中自我改进 skill，跨会话持久记忆（agent 自己维护 MEMORY.md）。这套「skill + MEMORY.md」的机制跟 SSS 现在的工作方式**结构上完全一致**，值得对照看它是怎么做 skill 自动生成的。部署面很宽：Linux / macOS / WSL2 / 原生 Windows / 安卓 Termux 都能跑，形态有 CLI、桌面应用、OpenAI 兼容 API 服务端、以及 Telegram/Discord/Slack/WhatsApp 等 20+ 消息平台网关，自带 cron 调度。注意分清免费与付费：软件本身零成本，Nous Portal（200+ 模型 + 工具网关）是 $20/月起的可选付费件，不用也行。

- [ ] **【Multica：开源自托管看板，把活派给 Claude Code / Codex / OpenClaw / Hermes / Copilot 就像派给同事】** | 来源：[The Agent Times 报道](https://theagenttimes.com/articles/multica-launches-free-open-source-platform-to-deploy-ai-agen-ba5f9740) | 🎯 Rubedo 凝华
  它不出模型，只做**编排层**：把现有的编码 Agent CLI 包进一个统一的看板界面，Agent 出现在 assignee 下拉框里，派单后自动执行多步任务、遇阻回报、自己更新状态，不用来回复制粘贴提示词。跟 8/02 收的「Copilot 堆叠会话」是同一个方向的两种解法——堆叠会话是串行小会话，Multica 是并行任务板。冷静的部分：**"100% 免费"只覆盖编排本身**，底下的 Claude Code / Codex 该烧 API 还是烧，真正零边际成本只有 OpenCode / OpenClaw 跑本地模型这条路。适合等 SSS 手上同时有 3 个以上并行改造任务时再上，单线程干活用不着。

- [ ] **【AEGIS：自托管个人 AI 编排平台，每个动作都过人工审批门 + 全量可回放审计】** | 来源：[项目主页与开发日志](https://hikmahtechnologies.com/aegis?ref=launches.uicomet.com) | 🎯 Rubedo 凝华
  7 月刚开源的一人自用系统，作者把私有系统改造成可分发版本的全过程都写在博客里，**设计决策的记录比代码本身更值钱**。几个直接可抄的点：(1) 一张 Postgres `interactions` 表 + 五种卡片 + Temporal 工作流，替代了原来每个业务域各写一套审批逻辑——"一个原语处理所有中断"；(2) Agent 的行为不写死在代码分支里，而是当成数据库里的能力标签，运行时解析、UI 里改，标签没主人时也不会崩；(3) GTD 用标签而非项目建模，Todoist 双向同步才跑得通；(4) 打了 publish 标签的待办 = 一条定时社媒帖，但必须有人在聊天里点一下审批卡才真的发出去。凝华要做「自动化但不失控」的 SOP，这套「人工审批门 + 可回放审计」是现成的范式。

- [ ] **【Codex 高阶玩法：让 Sol 当工头指挥 Luna Max 干活，同样额度产出翻倍】** | 来源：[aihot](https://x.com/AYi_AInotes/status/2083867265179537565) | 🎯 Rubedo 凝华
  很小但很实用的省钱技巧：在 `~/.codex/agents/` 下建一个 `luna-worker.toml` 子代理，模型设 `gpt-5.6-luna`、reasoning effort 拉满 max。分工是 Sol 负责拆任务和审代码（贵模型做贵活），具体实现自动委托给 Luna Max（便宜模型做重复劳动）。本质是**在单个工具内部做模型路由**，跟上面那条服务化路由是同一个降本逻辑的自用版。一条配置的事，值得当天试。

- [ ] **【Grok 支持分析任意视频】** | 来源：[aihot](https://x.com/elonmusk/status/2083800942927839307) | 🎯 Nigredo 馏析
  马斯克官宣 Grok 可以分析任何视频。对馏析的意义是多一条**不用先下载、不用先转写就能问内容**的通路，适合快速判断一条长视频值不值得走完整的下载→转写→提炼流程，当作前置筛子用。官宣只有一句话没有细节，能力边界（时长上限、是否读画面还是只读音轨、有没有速率限制）得自己实测一遍才知道能不能进管线。

- [ ] **【视频号 8 月创作者分层 2.0：中腰部专项，B 级占比 42%，内测样本播放量涨 217%】** ⚠️ 信源较弱待验证 | 来源：[自媒体转述闭门会](http://woshizmt.cn/category/shipinhao/3257.html) | 🎯 Rubedo 凝华
  据称 7 月底微信创作者闭门会释放的信号：8 月起分层从「看粉丝量」改成「综合价值评分」，由内容质量分 / 互动健康分 / 商业履约分 / 用户粘性分四维合成，划 S/A/B/C/D 五档；B 级（60–75 分）占创作者总量约 42%，1 万–50 万粉的中腰部会拿到流量加权、商单优先推荐、直播流量券。给的操作门槛是完播率 >45%、互动率 >5%、评论回复率 >60%。**必须打折看**：来源是自媒体转述的"参会创作者透露"，非官方公告，那个 217% 的内测涨幅尤其像营销话术。做法是等官方规则页更新再对齐，但「完播 + 互动 + 回复率」这三个指标本身跟 8/02 收的小红书算法硬指标方向一致，可以先按这个标准养内容习惯，不吃亏。

### ⚪ 了解

- [ ] **【OpenRouter 发布 LangChain 专用集成包，400+ 模型自动负载均衡与故障切换】** | 来源：[OpenRouter](https://openrouter.ai/blog/tutorials/langchain-chatopenrouter-setup) | 🎯 Citrinitas 熔知
  `langchain-openrouter`（Python）和 `@langchain/openrouter`（TS）两个包，LangChain 应用不用改造就能调 400+ 模型、70+ 提供商，ChatOpenRouter 自动处理负载均衡和故障切换，换模型只改一个 `provider/model` 字符串。熔知如果哪天要做"主模型挂了自动降级"的容错，这是最省事的接法。工具箱备件，用到再装。

- [ ] **【Google Flow Music 上线 Lyria 3.5：旋律结构、歌词遵循度、人声情感全面提升，可控节奏与时长】** | 来源：[Google DeepMind](https://deepmind.google/blog/were-launching-lyria-35-in-google-flow-music-with-advances-across-musicality-lyrics-vocals-and-creative-control) | 🎯 Nigredo 馏析
  跟 7/29 收的 Suno 音轨分离/MIDI 导出形成对照，短视频配乐这条链上又多一个选项，且**可控输出时长**这点对卡点剪辑很实用。放 ⚪ 是因为配乐目前不是瓶颈——但注意上面那条德国法院判 Suno 训练与输出双重侵权（8/02 已收），如果商用配乐的版权风险变成真问题，大厂自训模型的合规确定性会突然值钱。

- [ ] **【德里高等法院裁定 OpenAI 用 ANI 内容训练未侵权，与德国 Suno 判决结论相反】** | 来源：[IT之家](https://www.ithome.com/0/982/520.htm) | 🎯 Rubedo 凝华
  法官认定属于印度《版权法》里研究类"合理使用"例外，且 ANI 未能证明 ChatGPT 直接复制其受版权内容；法院还明说此刻发禁令会伤害印度本土 LLM 研发和大量免费用户。跟 8/02 收的德国 Suno 案（训练+输出双重侵权、责任归平台）**结论完全相反**，两条放一起看结论就一句话：**AI 内容的版权风险是按司法辖区切片的，没有全球统一答案**。对一人公司的实操含义——出海内容业务的版权尽调要按目标市场单独做，别拿一份结论走天下。

- [ ] **【Gary Marcus 泼冷水：OpenAI 新模型 Astra 数学惊艳，但推论犯了"合成谬误"】** | 来源：[Gary Marcus](https://garymarcus.substack.com/p/openais-amazing-but-vastly-oversold) | 🎯 Albedo 炼真
  核心论点值得记：**擅长某类数学 ≠ 擅长所有数学 ≠ 擅长科学 ≠ 擅长一切认知任务**。数学之所以先被攻破，是因为它能用符号工具廉价验证、能海量生成合成数据，而开放世界问题没有这两个条件。加上 OpenAI 没公布方法细节，外界压根无从评估真实意义。这套「能力泛化谬误」的拆法跟炼真判定证据链是同构的——**看到一个亮眼的单点指标，先问"验证条件是不是被特意挑过"**。可以直接抄进炼真的判定话术。

### 💡 探索

- [ ] **【Martha Stewart 联合创办 Hint：给房主的 AI 家居管理助手，免费上线、公开数据自动建房屋档案】** | 来源：[TechCrunch](https://techcrunch.com/2026/07/29/hint-a-new-ai-startup-co-founded-by-martha-stewart-offers-an-ai-assistant-for-homeowners) | 🎯 OpusMagnum 巨作
  产品本身不复杂：帮房主管维护计划、能耗、土壤与空气质量、保险理赔，存合同和文件还能问答；靠公开数据给每栋房子建档案，配主动维护提醒和一个"房屋评分"。iOS 免费、无订阅无广告，后面再上付费高级功能。值得放探索区是因为它同时押中三件事：**名人 IP 直接换用户获取成本**、**用公开数据预建档案换掉冷启动录入**、**用一个可量化的"评分"制造持续回访理由**。这三招跟 SSS 的建筑/室内设计本行是相邻领域——把"房屋档案"换成"户型档案 / 项目档案"，把"房屋评分"换成"空间合理性评分"，就是一个能长出来的 C 端切口。先当参照系看，不急着做。

- [ ] **【真实营收基准数据源：TrustMRR 收录 156 个 OpenClaw 项目 30 天合计 $379,631；国内 595 个 AI 单人项目收入分三层】** | 来源：[Claude Code 创业指南（含 TrustMRR 数据）](http://cnnetsun.cn/a/2544038) + [国内单人 AI 公司调研](http://www.xxmr.cn/news/58959) | 🎯 OpusMagnum 巨作
  两组用来**校准自己预期**的数字。海外：TrustMRR 截至 2026/3 收录 156 个基于 OpenClaw 的项目，成立最长不超 6 周，过去 30 天合计营收 $379,631，平均每个项目月入 $2,432，头部单月 $50,000——TrustMRR 这个平台本身也值得当"真实营收数据源"收藏。国内：孵化器联调 595 个 AI 单人项目，52.7% 月营收 7000 元以内（工具倒卖、无壁垒、同质化），39.1% 月营收 1–5 万（深耕单一行业 + 标准化 AI 交付流程），8.2% 月营收 5–30 万（自研垂直智能体 + 订阅制现金流）。**信源分级说明**：TrustMRR 是可查平台数据、可信度中上；国内 595 项目那组来自营销号转述的"孵化器联合调研"，没给原始报告，可信度低，只当量级参考不当依据。真正有用的是那条分层规律——**从"卖工具"升到"标准化交付流程"是月入 7000 到 1–5 万的分水岭，再升到"自研垂直智能体 + 订阅制"才是 5 万以上**。这跟熔知/凝华现在的路线是同一个方向，可以拿来反推优先级。
